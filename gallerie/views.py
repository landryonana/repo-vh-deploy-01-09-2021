...
def gallerie_add_image(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ext = str(image.image.name).split('.')[-1]
            image_name = f'image-de-{slugify(image.evangelisation)  }-publier-le-{image.publish}.{ext}'
...
def gallerie_add_image_gallerie(request):
    ...
    if request.method == 'POST':
        form = ImageFormUpdate(request.POST, files=request.FILES)
        if form.is_valid():
            image = form.save(commit=False)
...
def gallerie_update(request, pk):
    im = get_object_or_404(Image, pk=pk)
    context = dict()
    if request.method=='POST':
        form = ImageFormUpdate(instance=im, data=request.POST, files=request.FILES)
        if form.is_valid():
            image = form.save(commit=False)
            ext = str(image.image.name).split('.')[-1]
            image_name = f'image-de-{slugify(image.evangelisation)  }-publier-le-{image.publish}.{ext}'
        ...
        form = ImageFormUpdate(instance=im)
    context['form'] = form
    context['image'] = im
    context['active'] = 'gallerie'
